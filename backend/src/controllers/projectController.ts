...
export const getProjects = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    ...
    let whereClause: any = isSuperAdmin ? {} : { organizationId: req.user!.organizationId };
...
export const updateProject = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    ...
    // Verify ownership
    const existing = await prisma.project.findFirst({ where: { id: (req.params.id as string), organizationId: req.user!.organizationId }});
...
export const getProjectHealth = async (req: AuthRequest, res: Response): Promise<any> => {
  try {
    ...
    const issues = await prisma.issue.findMany({ where: { projectId: id } });
    const budgetRevisions = await prisma.budgetRevision.findMany({ where: { budget: { projectId: id } } });
    const expenses = await prisma.expense.findMany({ where: { projectId: id } });
    const invoices = await prisma.invoice.findMany({ where: { projectId: id, status: { not: 'REJECTED' } } });
    const risks = await prisma.projectRisk.findMany({ where: { projectId: id } });
    ...
    if (tasks.length > 0) {
      ...
      if (overdueRatio > 0.2) scheduleReason = `${overdue} tasks are overdue.`;
      else scheduleReason = 'Schedule is mostly on track.';
    }
    ...
    const breakdown = components.map(c => {
      if (c.available) {
        ...
        accumulatedScore += (c.score as number) * c.weight;
      }
      return {
        metric: c.name,
        available: c.available,
        score: c.available ? Math.round(c.score as number) : null,
